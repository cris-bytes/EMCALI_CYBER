...
def render(*args, **kwargs):
    ...
    with st.form("llm_enterprise_settings_form"):
        ...
        cfg["backend"] = st.selectbox(
            ...
            ["ollama", "anthropic", "gemini", "openai", "offline"],
            index=["ollama", "anthropic", "gemini", "openai", "offline"].index(cfg.get("backend", "ollama"))
            if cfg.get("backend", "ollama") in ["ollama", "anthropic", "gemini", "openai", "offline"] else 0,
        )
        ...
        st.markdown("**Aceleración Ollama**")
        gpu_options = [-1, 0, 1, 10, 99]
        gpu_labels = {-1: "Automático / Ollama decide", 0: "Forzar CPU", 1: "Intentar 1 capa GPU", 10: "Intentar 10 capas GPU", 99: "Intentar máximo GPU"}
        current_num_gpu = int(cfg.get("ollama_num_gpu", -1)) if str(cfg.get("ollama_num_gpu", -1)).lstrip('-').isdigit() else -1
        cfg["ollama_num_gpu"] = st.selectbox("Modo aceleración Ollama", gpu_options, index=gpu_options.index(current_num_gpu) if current_num_gpu in gpu_options else 0, format_func=lambda x: gpu_labels[x])
        cfg["gpu_monitoring_enabled"] = st.checkbox("Monitoreo GPU activo", value=bool(cfg.get("gpu_monitoring_enabled", True)))

        st.subheader("3. Nube y fallback")
        col6, col7, col8, col8b = st.columns(4)
        with col6:
        ...
        with col8b:
            cfg["fallback_anthropic"] = st.checkbox("Fallback Anthropic", value=bool(cfg.get("fallback_anthropic", False)))
        ...
        current_anthropic = str(cfg.get("anthropic_model", "claude-sonnet-4-6"))
        if current_anthropic not in CLAUDE_MODELS.values():
            current_anthropic = "claude-sonnet-4-6"
        claude_labels = list(CLAUDE_MODELS.keys())
        claude_values = list(CLAUDE_MODELS.values())
        cfg["anthropic_model"] = CLAUDE_MODELS[st.selectbox(
            "Modelo Anthropic / Claude",
            claude_labels,
            index=claude_values.index(current_anthropic),
            help="Opus: máximo razonamiento. Sonnet: balance. Haiku: baja latencia/costo."
        )]
        ...
        cfg["anthropic_api_key"] = st.text_input("Anthropic API Key", value=str(cfg.get("anthropic_api_key", "")), type="password")
